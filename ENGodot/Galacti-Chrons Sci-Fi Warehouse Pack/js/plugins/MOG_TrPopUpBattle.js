...
SpriteEnemyTrP.prototype.createIcon = function() {
    ...
    for (var i = 0; i < this._enemy._treasure.item.length; i++) {
		 ...
		 if (item) {
			 this._iconsitems[i] = new Sprite(this._iconImg) 
			 this._iconsitems[i].item = item;
			 this._iconsitems[i].index = i;
			 this._iconsitems[i].anchor.x = 0.5;
			 this._iconsitems[i].anchor.y = 1;
			 this.refreshIcons(this._iconsitems[i]);
			 this.addChild(this._iconsitems[i]);
		 };
	};
	this._iconsitems.sort(function(a, b){return b.intY-a.intY});
	this.children.sort(function(a, b){return b.intY-a.intY});
	for (var i = 0; i < this._iconsitems.length; i++) {
		 this.refreshWait(this._iconsitems[i],i,this._iconsitems.length);
	};
};
...
SpriteEnemyTrP.prototype.update = function() {
	...
	if (this._iconImg.isReady()) {
		for (var i = 0; i < this._iconsitems.length; i++) {
			if (this._iconsitems[i].visible) {this.updateSprites(this._iconsitems[i])};
		};
	};
};